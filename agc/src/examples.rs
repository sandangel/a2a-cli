...
pub const SEND_FIELDS_STATE_AND_ARTIFACTS: &str =
    r#"agc send "Summarise this PR" --fields "{status,artifacts}""#;

// ── Multi-turn conversation ───────────────────────────────────────────

/// Start a conversation — capture the contextId from the response.
pub const SEND_CAPTURE_CONTEXT: &str =
    r#"agc send "My name is San." --fields "{contextId,artifacts}""#;
...
pub const SEND_FIELDS_ID_STATE: &str = r#"agc send "Hello" --fields "{id,status}""#;
