...
var generateSkillsFlags struct {
	outputDir string
}

var generateSkillsCmd = &cobra.Command{
	Use:   "generate-skills",
	Short: "Generate SKILL.md files for AI coding tools",
	Long: `Generate skills/agc-shared/SKILL.md — a reference file that teaches AI coding
tools (Claude Code, Copilot, Cursor, etc.) how to use agc.

The file is derived from the live CLI binary: flags and commands come from
cobra (the same source as --help), so any code change is reflected automatically
when you re-run this command.`,
	Example: `  agc generate-skills
  agc generate-skills --output-dir .claude/skills`,
	RunE: runGenerateSkills,
}

func init() {
	generateSkillsCmd.Flags().StringVar(&generateSkillsFlags.outputDir, "output-dir", "skills",
		"Directory to write SKILL.md files into")
	rootCmd.AddCommand(generateSkillsCmd)
}

func runGenerateSkills(cmd *cobra.Command, args []string) error {
	outDir := generateSkillsFlags.outputDir
	if err := validate.SafeOutputDir(outDir); err != nil {
		return err
	}

	if err := writeSkill(outDir, "agc-shared", sharedSkillContent()); err != nil {
		return err
	}
	fmt.Fprintf(os.Stderr, "  wrote agc-shared/SKILL.md\n")
...
// --------------------------------------------------------------------------
// agc-shared — static template, version-substituted at generate time.
//
// Modelled after gws-cli: the shared skill is a hardcoded string so we have
// full control over formatting and content. No cobra introspection — that
// leaks internal types (<stringArray>) and meta-commands (generate-skills).
// Update this template when commands, flags, or protocol behaviour change.
// --------------------------------------------------------------------------

const sharedSkillTemplate = `---
name: agc-shared
version: __VERSION__
...
` + "```bash" + `
# Register an agent
agc agent add prod https://my-agent.example.com
agc agent use prod
...
func sharedSkillContent() string {
	return strings.ReplaceAll(sharedSkillTemplate, "__VERSION__", skillVersion)
}
...
func writeIndex(outDir string) error {
	...
	sb.WriteString("> Auto-generated by `agc generate-skills`. Do not edit manually.\n\n")
	sb.WriteString("## Core Reference\n\n")
	sb.WriteString("How to use agc with any A2A agent.\n\n")
	sb.WriteString("| Skill | Description |\n|-------|-------------|\n")
	sb.WriteString("| [agc-shared](./agc-shared/SKILL.md) | agc CLI reference: setup, sending messages, reading normalized responses, task management, and exit codes for AI tools. |\n\n")

	if err := os.MkdirAll(outDir, 0o755); err != nil {
		return fmt.Errorf("create output dir: %w", err)
	}
	return os.WriteFile(filepath.Join(outDir, "INDEX.md"), []byte(sb.String()), 0o644)
}

func writeSkill(baseDir, name, content string) error {
