...
fn apply_jq(value: &Value, filter: &str) -> Result<Vec<Value>> {
    ...
    let bytes = serde_json::to_vec(value).map_err(A2aCliError::Json)?;
    let input = read::parse_single(&bytes)
        .map_err(|e| A2aCliError::InvalidInput(format!("jq input error: {e}")))?;
    ...
    let modules = loader
        .load(&arena, program)
        .map_err(|e| A2aCliError::InvalidInput(format!("jq parse error: {e:?}")))?;

    let compiled = jaq_core::Compiler::default()
        .with_funs(funs)
        .compile(modules)
        .map_err(|e| A2aCliError::InvalidInput(format!("jq compile error: {e:?}")))?;
    ...
    compiled
        ...
        .map(|r| {
            let v = unwrap_valr(r)
                .map_err(|e| A2aCliError::InvalidInput(format!("jq runtime error: {e}")))?;
            // Val has no Serialize impl; round-trip through its JSON Display
            serde_json::from_str(&v.to_string()).map_err(A2aCliError::Json)
        })
        .collect()
}
